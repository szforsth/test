...
int arp(int argc,char *argv[])
{
	...
	while(1)
	{
		...
		if(ntohs(*(unsigned short*)(recv_msg+12))==0x0806 &&recv_msg[21] == 2)			//ARP应答
		{
...
void *send_arp_ask(void *arg)
{
	...
	unsigned char send_msg[1024] = {
		...
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //src_mac: 00:0c:29:75:a6:51
		...
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00,	//发送端的MAC地址
		0,  0,   0,  0,  				//发送端的IP地址
	...
	printf("Enter the NIC name:\n");
	char nic_name[IFNAMSIZ] = {0};
	fgets(nic_name, IFNAMSIZ, stdin);
	nic_name[strlen(nic_name)-1] = '\0';
	strncpy(ethreq.ifr_name, nic_name, IFNAMSIZ);	//指定网卡名称
	...
	char self_mac[18] = "";		//arp响应的MAC
	char self_ip[16] = "";		//arp响应的IP

	sprintf(self_mac, "%02x:%02x:%02x:%02x:%02x:%02x", \
			send_msg[22],send_msg[23],send_msg[24],send_msg[25],send_msg[26],send_msg[27]);
	sprintf(self_ip, "%d.%d.%d.%d", send_msg[28], send_msg[29], send_msg[30], send_msg[31]);
	printf("selfIP:%s - selfMAC:%s\n",self_ip, self_mac);
	...
	while(1)
	{
		...
		printf("Enter the target network segment:\n");
